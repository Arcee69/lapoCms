...
const DeleteJob = ({ handleClose, data }) => {
    const [loading, setLoading] = useState(false)
    // console.log(data?.id, "pink")

    const deleteJob = async () => {
        setLoading(true)
        await api.delete(appUrls?.CREATE_JOBS_URL + `/${data?.id}`)
        .then((res) => {
  ...
  return (
    <div className='flex flex-col bg-[#fff] w-8/12 md:w-[327px] xs:h-[150px] md:h-[250px] p-8'>
        <div className='flex flex-col text-center gap-3'>
            ...
            <p className='text-[#000] '>Once you click Delete the job can't be retrieve again</p>
        </div>
